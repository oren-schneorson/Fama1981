...
for period = sample_tab.period(1:end)'
    ...
    lib_results = fullfile(root, 'ML_FGLS_il_ZRD',...
    ...
    fpath_R_in = fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'ZRD', series_Rf, fname_R_in);
    ...
    % *******************************************************************
    if exist(fpath_ML, 'file') == 2
        result_ML = readtable(fpath_ML);
        theta_ML = result_ML.theta;
        beta_ML = result_ML.beta_rf;
    ...
    % *******************************************************************
    if exist(fpath_FGLS, 'file') == 2
        result_FGLS = readtable(fpath_FGLS);
        theta_FGLS = result_FGLS.theta;
        beta_FGLS = result_FGLS.beta_rf;
    ...
    fpath_fig = fullfile(root, 'gfx', 'IL', 'log-likelihood_ZRD', fname_fig);
    ...
    bar(grid_h, [acf_err_ML; acf_errw_ML; acf_err_1])
    legend({'err_{ML}', 'errw_{ML}', 'err_1'})
    ...
    if exist(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'ZRD', series_Rf), 'dir') ~= 2
        mkdir(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'ZRD', series_Rf))
    end
    ...
    writetable(array2table([theta_ML, beta_ML', R_sq_ML_diff,...
        summary_stats{1, 'mean'}, summary_stats{2, 'mean'},...
        summary_stats{1, 'rho_1'}, summary_stats{2, 'rho_1'},...
        ], 'VariableNames', vnames_ML_FGLS),...
        fpath_ML);
    writetable(array2table([theta_FGLS, beta_FGLS', R_sq_FGLS_diff,...
        summary_stats{1, 'mean'}, summary_stats{2, 'mean'},...
        summary_stats{1, 'rho_1'}, summary_stats{2, 'rho_1'},...
        ], 'VariableNames', vnames_ML_FGLS),...
        fpath_FGLS);
    ...
    fpath_R_out = fullfile('/home', username, 'R', 'Fama_Gibbons_1982', 'results', 'il', 'ZRD',...
    ...
    ***************************************************
    ...
    if exist(fpath_R_out, 'file') == 2
        ...
        fpath_comparing_KalmanFilter2CondNormal = fullfile('.', 'gfx',...
            'comparing_KalmanFilter2CondNormal', 'il', 'ZRD', series_Rf,...
    ...
    resid_ML = T_.Pi-beta_ML*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev)];
    alpha_t = [0; cumsum(Ev)] + mean(resid_ML);

    resid_FGLS = T_.Pi-beta_FGLS*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev)];
    alpha_FGLS_t = [0; cumsum(Ev_FGLS)] + mean(resid_FGLS);

    resid_1 = T_.Pi-1*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev1)];
    alpha_1_t = [0; cumsum(Ev1)] + mean(resid_1);

    EITB = alpha_t(1:end-1) + beta_ML * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB = T_.Pi(2:end)-EITB;

    EITB_FGLS = alpha_FGLS_t(1:end-1) + beta_FGLS * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB_FGLS = T_.Pi(2:end)-EITB_FGLS;

    EITB1 = alpha_1_t(1:end-1) + 1.0 * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB1 = T_.Pi(2:end)-EITB1;

    SSR = sum(UITB.^2);
    SSR_FGLS = sum(UITB_FGLS.^2);
    SSR_1 = sum(UITB1.^2);

    SS_tot = sum((T_.Pi(2:end)-mean(T_.Pi(2:end))).^2);
    R_sq = 1-SSR/SS_tot;
    R_sq_FGLS = 1-SSR/SS_tot;
    R_sq_1 = 1-SSR_1/SS_tot;

    RMSE = rmse(T_.Pi(2:end), EITB);
    RMSE_FGLS = rmse(T_.Pi(2:end), EITB_FGLS);
    RMSE_1 = rmse(T_.Pi(2:end), EITB1);
    ...
    clf
    hold on
    plot(T_.date(2:end), EITB)
    %plot(T_.date(2:end), EITB1)
    plot(T_.date, T_.Pi, 'k--')    
    %plot(T_.date, alpha_t, 'g')

    legend({'EITB', '\pi', })
    %legend({'EITB', 'EITB1', '\pi', })
    %legend({'EITB', 'EITB1', '\pi', '\alpha_t'})

    title(str4fig(series_Rf))
    
    export_fig(fullfile(root, 'gfx', 'IL', 'EITB', series_Rf, fname_fig))
    ...
    h=12;
    plot(T_.date(3+h:end), arrayfun(@(t) mean(UITB(t:t+h)), 1:N-h-1)')
    title(str4fig(series_Rf), sprintf('ML: E(UITB)=%.3f; acf1=%.3f; RMSE=%.3f',...
        mean(UITB), autocorr_(UITB, 1), RMSE))
    legend({'UITB', 'UITB-ma12'})
    
    export_fig(fullfile(root, 'gfx', 'IL', 'UITB', series_Rf, fname_fig))
    ...
    h=0;
    plot(T_.date(3+h:end), arrayfun(@(t) mean(UITB_FGLS(t:t+h)), 1:N-h-1)')
    h=12;
    plot(T_.date(3+h:end), arrayfun(@(t) mean(UITB_FGLS(t:t+h)), 1:N-h-1)')
    title(sprintf('FGLS: E(UITB)=%.3f; acf1=%.3f; RMSE=%.3f',...
        mean(UITB_FGLS), autocorr_(UITB_FGLS, 1), RMSE_FGLS))
    legend({'UITB', 'UITB-ma12'})
    ...
    h=0;
    plot(T_.date(3+h:end), arrayfun(@(t) mean(UITB1(t:t+h)), 1:N-h-1)')
    h=12;
    plot(T_.date(3+h:end), arrayfun(@(t) mean(UITB1(t:t+h)), 1:N-h-1)')
    title(sprintf('E(UITB1)=%.3f; acf1=%.3f; RMSE=%.3f',...
        mean(UITB1), autocorr_(UITB1, 1), RMSE_1))
    legend({'UITB', 'UITB-ma12'})
    %}
    ...
    if exist(lib_save_EITB, "dir") ~= 7
        mkdir(lib_save_EITB)
    end

    if exist(lib_save_UITB, "dir") ~= 7
        mkdir(lib_save_UITB)
    end
