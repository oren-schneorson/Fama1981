...
series_rr = 'TSB_ZRD_01Y.M'; % real rate
...
lib_rr = fullfile(lib_israel, 'TSB_ZRD', 'M', 'with_metadata');
...
for i = 1:size(data, 2)
    ...
    if strcmp(data(i).series, series_CPI)
        ...
        T_.date = T_.date + calmonths(floor(lag_inflation));
    end
...
% Sample period
period = sample_tab.period(4); % whole sample
...
idx_sample = idx_sample & T.date >= sample_tab.min_date(period);
idx_sample = idx_sample & T.date <= sample_tab.max_date(period);
T = T(idx_sample, :);
...
lib_results = fullfile(root, 'ML_FGLS_il_ZRD_oos',...
    freq, sprintf('li%.1f', lag_inflation), 'Rf');

vnames_ML_FGLS = [vnames_ML_FGLS, {'R_sq', 'mean_Pi', 'mean_Rf', 'rho1_Pi', 'rho1_Rf'}];
...
for t = 25:size(T, 1) % at least 24 observations
    idx_sample_oos = true(size(T, 1), 1);
    idx_sample_oos = idx_sample_oos & T.date >= min_date;
    idx_sample_oos = idx_sample_oos & T.date < T.date(t);
    % withold last observation, t
    T_ = T(idx_sample_oos, :);
    ...
    summary_stats = array2table(summary_stats, 'VariableNames',...
    ...
    L_fun_beta_1 = @(theta_, beta_) (L_fun(theta_, beta_) - L_fun(theta_, beta_+1e-9))./1e-9;
    L_fun_theta_1 = @(theta_, beta_) (L_fun(theta_, beta_) - L_fun(theta_+1e-9, beta_))./1e-9;
    ...
    precision = 1e-5;
    precision_theta = 1e-7;
    precision_beta = 1e-7;
    spectrum_theta = [theta_ML_t-.2, theta_ML_t+.2];
    spectrum_beta = [beta_ML_t-.5, beta_ML_t+.5];
    
    counter = 0;
    while abs(theta_ML_t-theta_prev) > precision
        counter = counter + 1;
        theta_prev = theta_ML_t;
        spectrum_theta = max(spectrum_theta, max_theta);
        spectrum_theta = min(spectrum_theta, min_theta);

        [beta_ML_t, ~, ~, ~] = my_fminunc(...
            @(beta_) -L_fun(theta_ML_t, beta_),...
            beta_ML_t, spectrum_beta, precision_beta, false);
        [theta_ML_t, ~, grid_theta, res_theta] = my_fminunc(...
            @(theta_) -L_fun(theta_, beta_ML_t),...
            theta_ML_t, spectrum_theta, precision_theta, true);

        spectrum_beta = [beta_ML_t-.1, beta_ML_t+.1];
        spectrum_theta = [theta_ML_t-.1, theta_ML_t+.1];

        fprintf('%s: Fixed point estimation: theta_{ML}=%.4f  beta_{ML}=%.4f\n',...
            T.date(t), theta_ML_t, beta_ML_t)
    end
    ...
    theta_prev = 100;
    precision = 1e-5;
    precision_theta = 1e-4;
    spectrum_theta = [theta_FGLS_t-.2, theta_FGLS_t+.2];
    
    counter = 0;
    while abs(theta_FGLS_t-theta_prev) > precision
        counter = counter + 1;
        theta_prev = theta_FGLS_t;

        spectrum_theta = max(spectrum_theta, min_theta);
        spectrum_theta = min(spectrum_theta, max_theta);

        [theta_FGLS_t, ~, grid_theta, res_theta] = my_fminunc(...
            @(theta_) -L_fun_FGLS(theta_), theta_FGLS_t,...
            spectrum_theta, precision_theta, false);
        beta_FGLS_t = beta_FGLS_fun(theta_FGLS_t);
        
        spectrum_theta = [theta_ML_t-.1, theta_ML_t+.1];       
        
        fprintf('%s: Fixed point estimation: theta_{FGLS}=%.4f  beta_{FGLS}=%.4f\n',...
            T.date(t), theta_FGLS_t, beta_FGLS_t)
    ...
    % STATS(1) % R_sq
    Y_hat_ML = X*beta_ML_t;
    Y_hat_FGLS = X*beta_FGLS_t;
    ...
    errw_ML = H_hat(theta_ML_t)*(Y-Y_hat_ML);
    errw_FGLS = H_hat(theta_FGLS_t)*(Y-Y_hat_FGLS);
    ...
    ***************************************************
    ...
    z_FGLS = err_FGLS; % set the observed signal as FLGS
    ...
    var_z_FGLS = var(z_FGLS); % sample variance of signal, unconditional
    %var_z_FGLS = arrayfun(@(t) var(z_FGLS(t:t+periods_ma)), 1:N-periods_ma)'; % sample variance of signal, conditional
    %var_z_FGLS = [var_z_FGLS(1)*ones(periods_ma, 1); var_z_FGLS];
    ...
    autocorr_z_FGLS = autocorr_(z_FGLS, 1);
    %autocorr_z_FGLS = arrayfun(@(t) autocorr_(z_FGLS(t:t+periods_ma), 1), 1:N-periods_ma)';
    %autocorr_z_FGLS = [autocorr_z_FGLS(1)*ones(periods_ma, 1); autocorr_z_FGLS];
    ...
    var_u_FGLS = - autocorr_z_FGLS .* var_z_FGLS;
    var_u1 = - autocorr_z1 .* var_z1;

    var_v = var_z - 2 * var_u;
    var_v_FGLS = var_z_FGLS - 2 * var_u_FGLS;
    var_v1 = var_z1 - 2 * var_u1;

    if var_v < 0
        warning('autocorrelation of signal lower than -0.50. Setting var_v to 10% of var_z')
        var_v = 0.1*var_z;
        var_v_FGLS = 0.1*var_z_FGLS;
        var_v1 = 0.1*var_z1;
        
        var_u = (var_z-var_v)/2;
        var_u_FGLS = (var_z_FGLS-var_v_FGLS)/2;
    ...
    % construct Sigma_z_FGLS
    Sigma_z_FGLS_ = 3 * eye(N) - ones(N); % [(2, -1, 0,..., 0); (0, 2, -1,...,0);...]
    Sigma_z_FGLS_ = Sigma_z_FGLS_-spdiags(zeros(N, 3), -1:1, Sigma_z_FGLS_);
    Sigma_z_FGLS = Sigma_z_FGLS_ .* var_u_FGLS + eye(N) .* var_v_FGLS;
    ...
    % construct Sigma_u_FGLS
    Sigma_u_z_FGLS = 2 * eye(N) - ones(N); % [(1, -1, 0,..., 0); (0, 1, -1,...,0);...]
    Sigma_u_z_FGLS = Sigma_u_z_FGLS-spdiags(zeros(N, 2), 0:1, Sigma_u_z_FGLS);
    Sigma_u_z_FGLS = Sigma_u_z_FGLS .* var_u_FGLS;
    ...
    if numel(var_z) ~= 1
        ...
        inv_Sigma_z_FGLS = Sigma_z_FGLS \ eye(N);
        ...
        inv_Sigma_z_FGLS = inv_sum(...
            Sigma_z_FGLS_*var_u_FGLS, eye(N)*var_v_FGLS,...
            inv_Sigma_z_/var_u_FGLS, eye(N)/var_v_FGLS...
            );
    ...
    E_u_given_z_FGLS = Sigma_u_z_FGLS * inv_Sigma_z_FGLS * (z_FGLS-0);
    E_u_given_z1 = Sigma_u_z1 * inv_Sigma_z1 * (z1-0);
    
    Delta_u = [E_u_given_z(1)-u_0; diff(E_u_given_z)];
    Delta_u_FGLS = [E_u_given_z_FGLS(1)-u_0; diff(E_u_given_z_FGLS)];
    Delta_u1 = [E_u_given_z1(1)-u_0; diff(E_u_given_z1)];

    Ev = z-Delta_u;
    Ev_FGLS = z_FGLS-Delta_u_FGLS;
    Ev1 = z1-Delta_u1;

    % average (minus) risk premium over the sample
    resid_ML = T_.Pi-beta_ML_t*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev)];
    alpha_t = [0; cumsum(Ev)] + mean(resid_ML);

    resid_FGLS = T_.Pi-beta_FGLS_t*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev)];
    alpha_FGLS_t = [0; cumsum(Ev_FGLS)] + mean(resid_FGLS);

    resid_1 = T_.Pi-1*(T_{:, {'Rf'}}-T_.rr) - [0; cumsum(Ev1)];
    alpha_1_t = [0; cumsum(Ev1)] + mean(resid_1);

    EITB_t = alpha_t(1:end-1) + beta_ML_t * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB_t = T_.Pi(2:end)-EITB_t;

    EITB_FGLS_t = alpha_FGLS_t(1:end-1) + beta_FGLS_t * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB_FGLS_t = T_.Pi(2:end)-EITB_FGLS_t;

    EITB1_t = alpha_1_t(1:end-1) + 1.0 * (T_{2:end, {'Rf'}}-T_.rr(2:end));
    UITB1_t = T_.Pi(2:end)-EITB1_t;

    SSR_t = sum(UITB_t.^2);
    SSR_FGLS_t = sum(UITB_FGLS_t.^2);
    SSR_1 = sum(UITB1_t.^2);

    SS_tot = sum((T_.Pi(2:end)-mean(T_.Pi(2:end))).^2);
    
    R_sq_t = 1-SSR_t/SS_tot;
    R_sq_FGLS_t = 1-SSR_FGLS_t/SS_tot;
    R_sq_1_t = 1-SSR_1/SS_tot;

    RMSE_t = rmse(T_.Pi(2:end), EITB_t);
    RMSE_FGLS_t = rmse(T_.Pi(2:end), EITB_FGLS_t);
    RMSE_1_t = rmse(T_.Pi(2:end), EITB1_t);

    EITB_t_oos = alpha_t(end) + beta_ML_t * (T{t, {'Rf'}}-T.rr(t)); % oos prediction
    EITB1_t_oos = alpha_1_t(end) + 1 * (T{t, {'Rf'}}-T.rr(t)); % oos prediction
    err_t_oos = T.Pi(t)-EITB_t_oos; % oos error
    err_1_t_oos = T.Pi(t)-EITB1_t_oos; % oos error

    res_t = {T.date(t), min_date,...
        T.Pi(t), EITB_t_oos, err_t_oos, alpha_t(end), err_1_t_oos, alpha_1_t(end),...
        N, K, theta_ML_t, theta_FGLS_t, beta_ML_t, beta_FGLS_t, RMSE_t, RMSE_FGLS_t, RMSE_1_t};
    vnames_res = {'date', 'min_date',...
        'Pi(t+1)', 'EITB_t_oos', 'err(t+1)_oos', 'alpha_t', 'err_1(t+1)_oos', 'alpha_1_t',...
        'N', 'K', 'theta_ML', 'theta_FGLS', 'beta_ML', 'beta_FGLS',...
        'RMSE_ML_t', 'RMSE_FGLS_t', 'RMSE_1_t'};
    res_t = array2table(res_t, 'VariableNames', vnames_res);
    res = [res; res_t];


end

fname_res = [series_Rf, '.xlsx'];
writetable(res, fullfile(root, 'ML_FGLS_il_ZRD_oos', fname_res))
clear metadata T_ T metadata
