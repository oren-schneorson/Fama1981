...
if contains(root, 'LiveEditor') || numel(root) == 0
    ...
    root = fullfile('C:\Users', username, 'Documents', 'MATLAB', 'Fama1981');
...
matlab_dir = fileparts(root);
% lib_data = fullfile('/media', username, 'D', 'data', 'BOI');
lib_data = fullfile(matlab_dir, 'data');
...
for period = sample_tab.period(1:end)'
    ...
    vnames = {'theta', 'beta_rf'};

    fname_ML = 'ML_';
    fname_FGLS = 'FGLS_';

    lib_results = fullfile(root, 'ML_FGLS_il', 'Rf');
    ...
    if incl_xr
        ...
        vnames = [vnames, {'beta_xr'}];
    end
    if incl_trgt
        lib_results = [lib_results, '_trgt'];        
        fname_ML = [fname_ML, 'trgt_'];
        fname_FGLS = [fname_FGLS, 'trgt_'];
        vnames = [vnames, {'beta_trgt'}];
    end

    vnames = [vnames, {'mean_Pi', 'mean_Rf', 'rho1_Pi', 'rho1_Rf'}];
    ...
    if exist(fpath_ML, 'file') == 2 && exist(fpath_FGLS, 'file') == 2
        %continue
    end
    ...
    summary_stats = [...
        [mean(T_.Pi), std(T_.Pi), arrayfun(@(h) autocorr_(T_.Pi, h), grid_h), ];...
        [mean(T_.Rf), std(T_.Rf), arrayfun(@(h) autocorr_(T_.Rf, h), grid_h), ]...
        ];
    summary_stats = array2table(summary_stats, 'VariableNames',...
        [{'mean', 'std'}, strcat('rho_', arrayfun(@num2str, grid_h, 'UniformOutput', false)),...
        ]);
    ...
    if incl_xr & incl_trgt
        lb_beta = [-2; -2; -10];
        ub_beta = [2; 2; 10];
        beta_ML = [-1.44, -.23, 0]';
        %beta_ML = [1, 0, 0]';
    elseif incl_xr && ~incl_trgt
        lb_beta = [-2; -2];
        ub_beta = [2; 2];
        beta_ML = [-1.44, -.23]';
        %beta_ML = [1, 0]';
    ...
    opts = optimoptions('fmincon', 'Display', 'iter', 'Algorithm', algo);
    while abs(theta_ML-theta_prev) > precision
        theta_prev = theta_ML;

        [beta_ML, ~, ~, ~, ~, ~] =...
            fmincon(@(beta_) -L_fun(theta_ML, beta_), beta_ML,...
            [], [], [], [], lb_beta, ub_beta, [], opts);
        
        [theta_ML, FVAL, EXITFLAG, OUTPUT, LAMBDA, GRAD] =...
            fmincon(@(theta_) -L_fun(theta_, beta_ML), theta_ML,...
            [], [], [], [], lb_theta, ub_theta, [], opts);
        ...
        fprintf('\nFixed point estimation: theta_{ML}=%.4f  beta_{ML}=%.4f\n',...
            theta_ML, beta_ML)
    end
    ...
    while abs(theta_FGLS-theta_prev) > precision
        ...
        [theta_FGLS,FVAL,EXITFLAG,OUTPUT,LAMBDA,GRAD] = fmincon(@(theta_) -L_fun_FGLS(theta_), theta_FGLS, [], [], [], [], lb_theta, ub_theta, [], opts);
    ...
    grid_theta = linspace(-.8, -.5, 26);
    grid_beta = linspace(-1.6, 0, 25);
    ...
    fpath_R_in = fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', series_Rf, fname_R_in);
    
    if exist(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', series_Rf), 'dir') ~= 2
        mkdir(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', series_Rf))
    end
    ...
    fpath_R_out = fullfile('/home', username, 'R', 'Fama_Gibbons_1982', 'results', 'il',...
