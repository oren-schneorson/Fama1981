...
%}
    ...
    % not using hac
    tab5_1_ = [N, B', t_stat', STATS(1), R_sq_adj, sqrt(sigma_hat), arrayfun(@(lag) autocorr_(R, lag), [1:5])];
    tab5_1 = [tab5_1; tab5_1_];

    
    % **********************************
    % REGRESSION (2)
    Y = RS;
    X = [ones(N, 1), dROC_lead];
    K = size(X, 2); % # of regressors
    ...
    t_stat = B./sqrt(diag(Sigma_hat));
    % not using hac
    tab5_2_ = [N, B', t_stat', STATS(1), R_sq_adj, sqrt(sigma_hat), arrayfun(@(lag) autocorr_(R, lag), [1:5])];
    tab5_2 = [tab5_2; tab5_2_];

    
    % **********************************
    % REGRESSION (3)
    Y = RS;
    X = [ones(N, 1), dPR, dPR_lead];
    K = size(X, 2); % # of regressors
    ...
    t_stat = B./sqrt(diag(Sigma_hat));
    % not using hac
    tab5_3_ = [N, B', t_stat', STATS(1), R_sq_adj, sqrt(sigma_hat), arrayfun(@(lag) autocorr_(R, lag), [1:5])];
    tab5_3 = [tab5_3; tab5_3_];

    
    % **********************************
    % REGRESSION (4)
    Y = RS;
    X = [ones(N, 1), dRGNP_lead];
    K = size(X, 2); % # of regressors
    ...
    t_stat = B./sqrt(diag(Sigma_hat));
    % not using hac
    tab5_4_ = [N, B', t_stat', STATS(1), R_sq_adj, sqrt(sigma_hat), arrayfun(@(lag) autocorr_(R, lag), [1:5])];
    tab5_4 = [tab5_4; tab5_4_];

    
    % **********************************
    % REGRESSION (5)
    Y = RS;
    X = [ones(N, 1), dRGNP_lead, dCX_NS_lead];
    K = size(X, 2); % # of regressors
    ...
    t_stat = B./sqrt(diag(Sigma_hat));
    % not using hac
    tab5_5_ = [N, B', t_stat', STATS(1), R_sq_adj, sqrt(sigma_hat), arrayfun(@(lag) autocorr_(R, lag), [1:5])];
    tab5_5 = [tab5_5; tab5_5_];
...
fpath_tab5 = fullfile(root, 'Fama1981', sprintf('tab5_%d.%s.csv', tab_num, freq));
eval(sprintf('tab = tab5_%d;', tab_num))
tab = [sample_tab, tab];
writetable(tab, fpath_tab5)    
