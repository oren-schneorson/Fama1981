...
lib_rr = fullfile(lib_israel, 'TSB_BAGR_GALIL', 'M', 'with_metadata');
lib_rr_hpfilter = fullfile(lib_israel, 'TSB_BAGR_GALIL_hpfilter', 'with_metadata');
...
series_rr = 'TSB_BAGR_GALIL_01Y.M'; % real rate
series_rr2 = 'TSB_BAGR_GALIL_02Y.M'; % real rate
series_rr3 = 'TSB_BAGR_GALIL_03Y.M'; % real rate
series_rr_hpfilter = 'TSB_BAGR_GALIL_01Y.M_hpfilter';
...
for i = 1:size(data, 2)
    ...
    if strcmp(data(i).series, series_rr) || strcmp(data(i).series, series_rr_hpfilter)
        T_.date.Day = 1;
        T_.date = T_.date+calmonths(1);
    end
    ...
    if strcmp(data(i).series, series_rr_hpfilter)
        T_{:, end} = fillmissing(T_{:, end}, 'linear');
    end
...
for period = sample_tab.period(1:end)'
    ...
    lib_results = fullfile(root, 'ML_FGLS_il_GALIL', 'Rf');
    ...
    fpath_R_in = fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'GALIL', series_Rf, fname_R_in);
    ...
    idx = ismember({data.series}, series_rr_hpfilter);
    ...
    fpath_fig = fullfile(root, 'gfx', 'IL', 'log-likelihood_GALIL', fname_fig);
    ...
    if exist(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'GALIL', series_Rf), 'dir') ~= 2
        mkdir(fullfile('/home' username, 'R', 'Fama_Gibbons_1982', 'data', 'il', 'GALIL', series_Rf))
    end
    ...
    fpath_R_out = fullfile('/home', username, 'R', 'Fama_Gibbons_1982', 'results', 'il', 'GALIL'...
