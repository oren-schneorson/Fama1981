...
T = outerjoin(T, arima_101(:, {'date', 'Pi_hat', 'epsilon_101'}),...
...
for period = sample_tab.period'
    ...
    epsilon_101 = epsilon_101/100*pcent_mult;
    Pi_hat = Pi_hat/100*pcent_mult;
...
fname = sprintf('data_tab_EI.%s.csv', freq);
fpath = fullfile(PycharmProjects_dir, 'Stocks_inflation', 'data', fname);
writetable(tab_EI, fpath)

fname = sprintf('data_tab_UI.%s.csv', freq);
fpath = fullfile(PycharmProjects_dir, 'Stocks_inflation', 'data', fname);
writetable(tab_UI, fpath)

fname = sprintf('data_tab_EI_UI.%s.csv', freq);
fpath = fullfile(PycharmProjects_dir, 'Stocks_inflation', 'data', fname);
