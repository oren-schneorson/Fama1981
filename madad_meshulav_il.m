...
for i = 1:numel(d)
    ...
    fpath = fullfile(lib_ns, d(i).name);
    T_ = readtable(fpath);
...
total = {...
    'AM_NSTOCK_ISKIS_Q_N',...    % Structures, corporate sector
    'AM_NSTOCK_ISKIM_Q_N',... % Equipment, corporate sector
    'AM_NSTOCK_TOTALS_Q_N',...    % Structures
    'AM_NSTOCK_TOTALM_Q_N',... % Equipment
    %'NCBNIPPCCB',...        % Intellectual Property Products, didn't find equivalent
    %'BOGZ1FL105013865Q',... % Structures Equipment and Intellectual Property Products, didn't find equivalent
    %'BOGZ1FL105013365Q',... % software, didn't find equivalent
    %'BOGZ1FL105013465Q',... % R&D, didn't find equivalent
    %'BOGZ1FL105013565Q',... % entertainment, didn't find equivalent
};


% should be included in TOTAL
residential = {...
    'AM_NSTOCK_BILDRES_Q_N',...   % Structures
    'AM_NSTOCK_BILDRES_Q_N',... % Equipment
    };
...
return
%plot(T.date, T{:, non_residential(4)}, 'LineWidth', 3, 'Color', 'r')
legend({...
    metadata.desc{ismember(metadata.series, grp(1))},...
    metadata.desc{ismember(metadata.series, grp(2))},...
    })
