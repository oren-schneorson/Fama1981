...
for period = sample_tab.period(1:end)'
    ...
    lib_results = fullfile(root, 'ML_FGLS_il', freq, sprintf('li%.1f', lag_inflation), 'Rf');
    ...
    %{
    % python integration didn't work.
    writematrix(X, 'X.csv')
    writematrix(Y, 'Y.csv')
    %algo = 'L-BFGS-B';
    %matlab_output = pyrunfile('minimize.py', 'matlab_output', algo='L-BFGS-B')
    %}
    ...
    while abs(theta_FGLS-theta_prev) > precision
        ...
        res_ML = readtable(fpath_ML);
        res_FGLS = readtable(fpath_FGLS);

        theta_ML = res_ML{1, vnames_ML_FGLS(1)};
        beta_ML = res_ML{1, vnames_ML_FGLS(2:end-4)}';
        theta_FGLS = res_FGLS{1, vnames_ML_FGLS(1)};
        beta_FGLS = res_FGLS{1, vnames_ML_FGLS(2:end-4)}';
    ...
    min_theta = min(-2.5, min(theta_FGLS, theta_ML)*1.1);
    max_theta = -.6;
    ...
    fpath_fig = fullfile(root, 'gfx', 'IL', 'log-likelihood', fname_fig);
    ...
    stats = [mean(T_.Pi), mean(T_.Rf), autocorr_(T_.Pi, 1), autocorr_(T_.Rf, 1)];
    
    res_ML = array2table([theta_ML, beta_ML, R_sq_ML_diff, stats],...
        'VariableNames', vnames_ML_FGLS);
    res_FGLS = array2table([theta_FGLS, beta_FGLS, R_sq_FGLS_diff, stats],...
        'VariableNames', vnames_ML_FGLS);

    writetable(res_ML, fpath_ML)
    writetable(res_FGLS, fpath_FGLS)
    continue
