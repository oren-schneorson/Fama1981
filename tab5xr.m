...
for i = 1:size(data, 2)
    ...
    elseif strcmp(data(i).series, series_Rf)
        T_.date = T_.date - calmonths(lead_Rf);
    end
...
for period = sample_tab.period'
    ...
    EITB = T.EITB*pcent_mult;
    UITB = T.UITB*pcent_mult;
    ...
    if strcmp(freq, 'Q')  && strcmp(series_Rf, 'RF_FF')
        Rf = Rf*3;
    elseif strcmp(freq, 'A') && strcmp(series_Rf, 'RF_FF')
        Rf = Rf*12;
    elseif strcmp(freq, 'Q') && ~strcmp(series_Rf, 'RF_FF')
        % pass
    elseif strcmp(freq, 'A') && ~strcmp(series_Rf, 'RF_FF')
        % pass
    else
...
fpath_table5 = fullfile(root, 'Fama1981', sprintf('table5xr_%d.%s.csv', tab_num, freq));
